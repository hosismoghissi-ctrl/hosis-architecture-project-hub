...
function renderDashboard(){
  ...
  var html='<section class="hero-strip dashboard-hero company-dashboard-header" style="--dashboard-image:url(\''+esc(company.banner)+'\')"><div class="company-hero-logo">'+(company.logo?'<img src="'+esc(company.logo)+'" alt="'+esc(company.name)+' logo">':icon("building-2"))+'</div><div class="hero-copy"><div class="eyebrow">'+esc(company.name.toUpperCase())+'</div><h2>'+esc(company.welcome)+'</h2><p>'+esc(company.summary)+'</p></div><div class="hero-actions"><button class="ghost-button" data-go-gallery>'+icon("panels-top-left")+'Open Projects</button><button class="ghost-button" data-edit-workspace>'+icon("settings-2")+'Admin Settings</button></div></section>'+
    ...
    '<div class="control-center-grid">'+renderDashboardProjects(projects)+renderTeamOverview()+renderSchedulePreview(projects)+
    '<section class="panel dashboard-stages" data-dashboard-module="stages"><div class="panel-head"><div><h3>Projects by Stage</h3><small>Project scope across the portfolio</small></div></div><div class="stage-bars">'+Object.keys(STAGES).map(function(k){var pct=projects.length?Math.round(stageCounts[k]/projects.length*100):0;return '<div><div class="stage-bar-head"><span>'+icon(STAGES[k].icon)+' '+esc(STAGES[k].label)+'</span><b>'+stageCounts[k]+'</b></div><div class="stage-track"><div class="stage-fill stage-'+k+'" style="width:'+pct+'%"></div></div></div>'}).join("")+'</div></section>'+
    renderDashboardTasks(projects)+'</div>'+renderDashboardMembers();
  document.getElementById("content").innerHTML=html;document.getElementById("content").classList.add("admin-dashboard-refined");bindDashboardTasks();bindMemberFilters();bindCommon();
  document.querySelector('[data-add-dashboard-member]').onclick=function(){openEditor(null,"memberCreate",-1,null);};
...
function renderTeamOverview(){
  return '<section class="panel team-overview" data-dashboard-module="team"><div class="panel-head"><div><h3>Team Overview</h3><small>'+Object.keys(USERS).length+' members · studio workload</small></div></div><div class="team-overview-head"><span>Member</span><span>Active Projects</span><span>Open Tasks</span><span>Overdue Tasks</span><span>Upcoming Deadline</span></div><div class="team-overview-list">'+Object.keys(USERS).map(function(id){var user=USERS[id],stats=memberStats(id);return '<button class="team-overview-row" data-member-filter="'+id+'"><span class="team-person">'+memberAvatar(user)+'<span><strong>'+esc(user.name)+'</strong><small>'+esc(user.role)+'</small></span></span><span class="compact-team-metrics"><span><b>'+stats.projects.length+'</b> projects</span><span><b>'+stats.open+'</b> open</span><span class="'+(stats.overdue?'metric-alert':'')+'"><b>'+stats.overdue+'</b> overdue</span></span><span class="team-deadline">'+(stats.upcoming?'Next: '+formatDate(stats.upcoming.date):'No upcoming deadline')+'</span></button>';}).join("")+'</div><button class="module-footer" data-open-members>Manage members '+icon("arrow-up-right")+'</button></section>';
}
function memberAvatar(member){return '<span class="member-photo">'+(member.photo?'<img src="'+esc(member.photo)+'" alt="">':'<b>'+esc(initials(member.name))+'</b>')+'</span>';}
function renderDashboardProjects(projects){
  return '<section class="panel dashboard-project-summary" data-dashboard-module="projects"><div class="panel-head"><div><h3>Projects</h3><small>'+projects.length+' active projects</small></div></div><div class="compact-project-list">'+(projects.length?projects.map(function(p){return '<button data-project="'+p.id+'"><small>'+esc(p.number)+'</small><strong>'+esc(p.name)+'</strong><span>'+esc(p.status)+' · '+esc(p.priority)+' priority</span></button>';}).join(""):'<p class="inline-empty">No active projects.</p>')+'</div><button class="module-footer" data-go-gallery>View all projects '+icon("arrow-up-right")+'</button></section>';
}
function renderSchedulePreview(projects){
  var bounds=timelineBounds(projects),today=isoDate(new Date()),showToday=new Date(today)>=bounds.start&&new Date(today)<=bounds.end;
  return '<section class="panel dashboard-schedule" data-dashboard-module="schedule"><div class="panel-head"><div><h3>Project Schedule</h3><small>Stage dates & overlaps</small></div></div><div class="compact-schedule"><div class="compact-schedule-scale"><span>'+formatDate(isoDate(bounds.start))+'</span><span>'+formatDate(isoDate(bounds.end))+'</span></div>'+(projects.length?projects.map(function(p){return '<button class="compact-schedule-row" data-project="'+p.id+'"><strong>'+esc(p.number)+'</strong><small>'+esc(p.name)+'</small><span class="compact-schedule-track">'+p.schedule.map(function(s){var left=timelinePosition(s.start,bounds),width=Math.max(1,timelinePosition(s.end,bounds)-left);return '<span class="compact-stage stage-'+esc(s.stage)+'" style="left:'+left+'%;width:'+width+'%" title="'+esc(stageLabel(s.stage)+' · '+s.start+' – '+s.end)+'"></span>';}).join("")+(showToday?'<i class="compact-today" style="left:'+timelinePosition(today,bounds)+'%" title="Today"></i>':'')+'</span></button>';}).join(""):'<p class="inline-empty">No stages scheduled.</p>')+'</div><button class="module-footer" data-go-schedule>Open full timeline '+icon("arrow-up-right")+'</button></section>';
}
function renderDashboardMembers(){
  return '<section class="dashboard-members" aria-labelledby="dashboardMembersTitle"><div class="section-title"><div><span class="section-kicker">PEOPLE & WORKLOAD</span><h2 id="dashboardMembersTitle">Members</h2><p>'+Object.keys(USERS).length+' people in the workspace</p></div><button class="primary-button" data-add-dashboard-member>'+icon("user-plus")+'Add Member</button></div><div class="dashboard-member-grid">'+Object.keys(USERS).map(function(id){var member=USERS[id],stats=memberStats(id);return '<button class="dashboard-member-card" data-member-filter="'+id+'">'+memberAvatar(member)+'<strong>'+esc(member.name)+'</strong><span class="member-job-title">'+esc(member.role)+'</span><span class="member-card-metrics"><span><b>'+stats.projects.length+'</b>Active projects</span><span><b>'+stats.open+'</b>Open tasks</span><span class="'+(stats.overdue?'metric-alert':'')+'"><b>'+stats.overdue+'</b>Overdue tasks</span></span></button>';}).join("")+'</div></section>';
}
function renderMemberWorkspace(userId){
  ...
  var html='<section class="member-personal-header" style="--member-banner:url(\''+esc(member.banner)+'\')"><div class="member-header-profile"><span class="member-header-photo">'+(member.photo?'<img src="'+esc(member.photo)+'" alt="'+esc(member.name)+'">':'<b>'+esc(initials(member.name))+'</b>')+'</span><div><span class="section-kicker">MEMBER WORKSPACE</span><h2>Welcome, '+esc(member.name.split(" ")[0])+'</h2><strong>'+esc(member.name)+'</strong><p>'+esc(member.role)+' · '+esc(member.welcome)+'</p></div></div><div class="hero-actions">'+(state.role==="admin"?'<button class="ghost-button" data-edit-member="'+userId+'">'+icon("pencil")+'Edit profile</button><button class="ghost-button" data-member-filter="">'+icon("building-2")+'Company View</button>':canSelfEdit?'<button class="ghost-button" data-edit-self>'+icon("image")+'Edit my images</button>':'')+'</div></section>'+
    ...
    '<section data-member-section="projects"><div class="section-title dashboard-project-title"><div><span class="section-kicker">ASSIGNED PORTFOLIO</span><h2>My Projects</h2><p>'+projects.length+' projects assigned to '+esc(member.name)+'.</p></div><button class="secondary-button" data-go-gallery>View all</button></div><div class="project-grid dashboard-projects">'+(projects.length?projects.map(projectCard).join(""):'<p class="inline-empty">No projects assigned yet.</p>')+'</div></section>'+
    '<div class="member-primary-section" data-member-section="tasks"><div class="section-title compact"><div><span class="section-kicker">PRIMARY WORKLOAD</span><h2>My Tasks Across All Projects</h2><p>Every open commitment from all assigned projects, ordered by priority and due date.</p></div></div>'+renderDashboardTasks(projects,"My Tasks Across All Projects")+'</div>'+
    '<div class="member-workspace-grid"><div data-member-section="deadlines">'+renderMemberDeadlines(stats.deadlines)+'</div><div data-member-section="meetings">'+renderMemberMeetings(meetings)+'</div></div>'+
    '<section class="panel timeline-preview dashboard-schedule" data-member-section="schedule"><div class="panel-head"><div><h3>My Schedule</h3><small>Stage dates across all assigned projects</small></div><button data-go-schedule>Open full schedule</button></div>'+renderTimeline(projects,true)+'</section><div data-member-section="activity">'+renderMemberActivity(activity)+'</div>';
...
function editorConfig(project,kind,index,stageKey){
  ...
  if(kind==="memberProfile"){
    ...
    return {title:"Edit "+item.name,description:"Admin can manage this member’s identity and personal header without changing company branding.",fields:[{name:"name",label:"Display Name",value:item.name},{name:"role",label:"Job Title",value:item.role},{name:"email",label:"Email",type:"email",value:item.email},{name:"photo",label:"Profile Image URL",type:"url",value:item.photo,required:false,wide:true},{name:"banner",label:"Personal Header Image URL",type:"url",value:item.banner,required:false,wide:true},{name:"welcome",label:"Personal Welcome Text",type:"textarea",value:item.welcome,wide:true}],validate:function(v){return (!v.photo||/^https:\/\/\S+$/i.test(v.photo))&&(!v.banner||/^https:\/\/\S+$/i.test(v.banner))?"":"Use secure HTTPS image URLs for the profile and banner.";},save:function(v){Object.assign(item,v);item.initials=initials(item.name);}};
  }
  if(kind==="memberCreate")return {title:"Add Member",description:"Add a person to this demo workspace. Project assignments can be managed in each Project Team. This does not send an invitation.",fields:[{name:"name",label:"Display Name",value:""},{name:"role",label:"Job Title",value:""},{name:"email",label:"Email",type:"email",value:""},{name:"photo",label:"Profile Image URL",type:"url",value:"",required:false,wide:true},{name:"banner",label:"Personal Header Image URL",type:"url",value:"",required:false,wide:true},{name:"welcome",label:"Personal Welcome Text",type:"textarea",value:"Your projects, people and coordination in one place.",wide:true}],validate:function(v){if(!v.name.trim()||!v.role.trim())return "Enter a name and job title.";if(Object.keys(USERS).some(function(id){return USERS[id].email.toLowerCase()===v.email.trim().toLowerCase();}))return "A member with this email already exists.";return [v.photo,v.banner].every(function(url){return !url||/^https:\/\/\S+$/i.test(url);})?"":"Use secure HTTPS image URLs.";},save:function(v){var id="member-"+Date.now().toString(36);state.members[id]=Object.assign({},v,{name:v.name.trim(),email:v.email.trim(),id:id,workspaceId:state.workspace.id,initials:initials(v.name),color:"#d8e3ed"});USERS=state.members;}};
  if(kind==="memberSelf"){
    item=USERS[stageKey];if(!item||state.userId!==stageKey)return null;
    var selfFields=[];if(state.workspace.permissions.memberCanEditPhoto)selfFields.push({name:"photo",label:"My Profile Image URL",type:"url",value:item.photo,required:false,wide:true});if(state.workspace.permissions.memberCanEditBanner)selfFields.push({name:"banner",label:"My Personal Header Image URL",type:"url",value:item.banner,required:false,wide:true});
    return {title:"Edit My Header Images",description:"These images belong only to your personal workspace. Company branding remains admin-controlled.",fields:selfFields,validate:function(v){return Object.keys(v).every(function(key){return !v[key]||/^https:\/\/\S+$/i.test(v[key]);})?"":"Use secure HTTPS image URLs.";},save:function(v){Object.assign(item,v);applyRoleNavigation(item);}};
  }
...
function saveEditor(form){
  ...
  if(kind==="memberCreate"){populateWelcomeUsers();renderDashboard();refreshIcons();}
}
